...
for dir in tests/* ; do
  ...
  name=$(basename "$dir")
  dir_abs="$(cd "$dir" && pwd)"
  poseidon_target="$dir_abs/target_poseidon2"
  mkdir -p "$poseidon_target"

  echo "► building $name (poseidon2)"
  pushd "$dir" >/dev/null
  ...
  cp "$json" "$poseidon_target/${name}.json"
  cp "$gz" "$poseidon_target/${name}.gz"

  bb prove -b "$poseidon_target/${name}.json" -w "$poseidon_target/${name}.gz" -o "$poseidon_target" \
    --scheme ultra_honk --oracle_hash poseidon2 --output_format bytes_and_fields

  bb write_vk -b "$poseidon_target/${name}.json" -o "$poseidon_target" \
    --scheme ultra_honk --oracle_hash poseidon2 --output_format bytes_and_fields
